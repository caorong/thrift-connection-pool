...
class ConnectionPool(object):
    ...
    # ms
    DEFAULT_NETWORK_TIMEOUT = 60 * 1000
    DEFAULT_POOL_SIZE = 100

    def __init__(self, host, port, iface_cls, size=DEFAULT_POOL_SIZE, async=False,
        ...
        self.transport_wrapper_cls = transport_wrapper_cls
    ...
    def _create_thrift_connection(self):
        socket = self.transport_cls(self.host, self.port)
        if self.network_timeout > 0 and callable(getattr(socket, "setTimeout", None)):
            socket.setTimeout(self.network_timeout)
        transport = self.transport_wrapper_cls(socket)
        protocol = self.protocol_cls(transport)
